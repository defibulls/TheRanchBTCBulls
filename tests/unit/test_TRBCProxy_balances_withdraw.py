...
def test_TRBCProxy_balances_withdraw():
    ...
    assert mocked_usdc.balanceOf(proxy_TRBC) == ((5 * 350) * (10 ** 6))
    assert proxy_TRBC.btcMinersSafeBalance.call() ==   ((5 * 350) * (10 ** 6)) * .90
    assert proxy_TRBC.hostingSafeBalance.call() == ((5 * 350) * (10 ** 6)) * 0.05
    assert proxy_TRBC.dailyRaffleBalance.call() ==  ((5 * 350) * (10 ** 6)) * 0.03
    assert proxy_TRBC.USDCRewardsBalance.call() == ((5 * 350) * (10 ** 6)) * 0.05
    
    assert proxy_TRBC.getNumberOfRafflePlayers() == 5
    assert proxy_TRBC.totalSupply() == 5
    ...
    assert proxy_TRBC.getUsdcBalanceForAddress({"from":coreTeam1}) == ((5 * 350) * (10 ** 6)) * .01
    assert proxy_TRBC.getUsdcBalanceForAddress({"from":coreTeam2}) == ((5 * 350) * (10 ** 6)) * .01
    ...
    tx_withdraw_btc_miner_fund = proxy_TRBC.withdrawBtcMinersSafeBalance({"from": multisig})
    ...
    assert mocked_usdc.balanceOf(proxy_TRBC) == ((5 * 350) * (10 ** 6)) - btc_miner_fund
    ...
    tx_withdraw_hosting_safe = proxy_TRBC.withdrawHostingSafeBalance({"from": multisig})
    ...
    assert mocked_usdc.balanceOf(proxy_TRBC) == ((5 * 350) * (10 ** 6)) - (btc_miner_fund + hosting_safe_amt)
    assert proxy_TRBC.btcMinersSafeBalance.call() ==   0
    assert proxy_TRBC.dailyRaffleBalance.call() ==  ((5 * 350) * (10 ** 6)) * 0.03
