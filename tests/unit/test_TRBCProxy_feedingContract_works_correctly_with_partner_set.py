...
def test_TRBCProxy_feedingContract_works_correctly_with_partner_set():
    ...
    proxy_TRBC.setPartnerAddress(person_3,{"from": person_4})
    assert proxy_TRBC.myPartner(person_4) == person_3
    ...
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": person_3}) == ((1 * 350) * (10 ** 6)) * 0.02
    ...
    proxy_TRBC.withdrawUsdcBalance({"from": person_3})
    ...
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": coreTeam1}) == 150
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": coreTeam2}) == 150
    ...
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": person_1}) == ((1 * 350) * (10 ** 6)) * 0.02
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": person_2}) == 0
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": person_3}) == 50
    assert proxy_TRBC.getUsdcBalanceForAddress({"from": person_4}) == 950
