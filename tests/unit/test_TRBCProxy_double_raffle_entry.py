...
def test_TRBCProxy_double_raffle_entry():
    ...
    assert mocked_usdc.balanceOf(proxy_TRBC) == 0
    assert proxy_TRBC.btcMinersSafeBalance.call() ==   0
    assert proxy_TRBC.hostingSafeBalance.call() == 0
    assert proxy_TRBC.dailyRaffleBalance.call() ==  0
    assert proxy_TRBC.USDCRewardsBalance.call() == 0    
    assert proxy_TRBC.getNumberOfRafflePlayers() == 0
    assert proxy_TRBC.totalSupply() == 0
    assert proxy_TRBC.getUserAlreadyInDailyRaffleStatus(person_1) == False
    ...
    assert proxy_TRBC.balanceOf(proxy_TRBC) == 0
    assert mocked_usdc.balanceOf(proxy_TRBC) == ((2 * 350) * (10 ** 6))
    assert proxy_TRBC.btcMinersSafeBalance.call() ==   ((2 * 350) * (10 ** 6)) * .90
    assert proxy_TRBC.hostingSafeBalance.call() == ((2 * 350) * (10 ** 6)) * 0.05
    assert proxy_TRBC.dailyRaffleBalance.call() ==  ((2 * 350) * (10 ** 6)) * 0.03
    assert proxy_TRBC.USDCRewardsBalance.call() == ((2 * 350) * (10 ** 6)) * 0.05
    
    assert proxy_TRBC.getNumberOfRafflePlayers() == 1
    assert proxy_TRBC.totalSupply() == 2
    ...
    amt = 2
    mocked_usdc.approve(proxy_TRBC.address, price_needed(amt),{"from":person_1})
    tx2 = proxy_TRBC.mint(amt,raffleEntryBool,{"from": person_1, "value":  price_needed(amt)})
    ...
    assert proxy_TRBC.balanceOf(proxy_TRBC) == 0
    assert mocked_usdc.balanceOf(proxy_TRBC) == ((4 * 350) * (10 ** 6))
    assert proxy_TRBC.btcMinersSafeBalance.call() ==   ((4 * 350) * (10 ** 6)) * .90
    assert proxy_TRBC.hostingSafeBalance.call() == ((4 * 350) * (10 ** 6)) * 0.05
    assert proxy_TRBC.dailyRaffleBalance.call() ==  ((4 * 350) * (10 ** 6)) * 0.03
    assert proxy_TRBC.USDCRewardsBalance.call() == ((4 * 350) * (10 ** 6)) * 0.05
    
    assert proxy_TRBC.getNumberOfRafflePlayers() == 1
    assert proxy_TRBC.totalSupply() == 4
    assert proxy_TRBC.getRafflePlayer(0,{"from": multisig}) == person_1
    assert proxy_TRBC.getUserAlreadyInDailyRaffleStatus(person_1) == True
