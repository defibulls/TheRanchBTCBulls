...
def test_funding_and_withdrawal_of_AirDrop_contract():
    ...
    contract_balance = TheRanchBullsAirDrop.getBalance({"from": owner})
    ...
    mocked_usdc.transfer(TheRanchBullsAirDrop.address,funds_deposited_owner, {"from": owner})


    assert TheRanchBullsAirDrop.getBalance({"from": owner}) == 0
    assert TheRanchBullsAirDrop.checkTokenBalance(mocked_usdc.address)  == funds_deposited_owner
    assert TheRanchBullsAirDrop.checkTokenBalance(mocked_dai.address)  == 0
    ...
    mocked_dai.transfer(TheRanchBullsAirDrop.address,funds_deposited_person_1_DAI, {"from": person_1})
    TheRanchBullsAirDrop.fund({"from": person_1, "value": funds_deposited_person_1_ETH})

    assert TheRanchBullsAirDrop.getBalance({"from": owner}) == funds_deposited_person_1_ETH
    assert TheRanchBullsAirDrop.checkTokenBalance(mocked_usdc.address)  == funds_deposited_owner
    assert TheRanchBullsAirDrop.checkTokenBalance(mocked_dai.address)  == funds_deposited_person_1_DAI
    ...
    contract_eth_start = TheRanchBullsAirDrop.getBalance({"from": owner})
    contract_usdc_start = TheRanchBullsAirDrop.checkTokenBalance(mocked_usdc.address) 
    contract_dai_start = TheRanchBullsAirDrop.checkTokenBalance(mocked_dai.address)
    ...
    TheRanchBullsAirDrop.withdraw(ETH_5K, {"from": owner})
    TheRanchBullsAirDrop.withdrawToken(mocked_usdc.address, USDC_5K, {"from": owner})
    TheRanchBullsAirDrop.withdrawToken(mocked_dai.address, DAI_5K, {"from": owner})

    assert TheRanchBullsAirDrop.getBalance({"from": owner}) == contract_eth_start - ETH_5K
    assert TheRanchBullsAirDrop.checkTokenBalance(mocked_usdc.address) == contract_usdc_start - USDC_5K
    assert TheRanchBullsAirDrop.checkTokenBalance(mocked_dai.address) == contract_dai_start - DAI_5K
    ...
    with pytest.raises(exceptions.VirtualMachineError):
        TheRanchBullsAirDrop.withdraw(ETH_5K, {"from": person_1})

    with pytest.raises(exceptions.VirtualMachineError):
        TheRanchBullsAirDrop.withdrawToken(mocked_usdc.address, USDC_5K, {"from": person_1})

    with pytest.raises(exceptions.VirtualMachineError):
        TheRanchBullsAirDrop.withdrawToken(mocked_dai.address, DAI_5K, {"from": person_1})
