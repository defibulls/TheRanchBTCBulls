...
def test_TRBCProxy_safe_addresses():
    ...
    current_hostingSafe_address = proxy_TRBC.hostingSafe.call()
    current_btcMinersSafe_address = proxy_TRBC.btcMinersSafe.call()
    ...
    with pytest.raises(exceptions.VirtualMachineError):
        proxy_TRBC.setSafeAddresses('0x0000000000000000000000000000000000000000',btcMinersSafe ,{"from": multisig})
        
    with pytest.raises(exceptions.VirtualMachineError):
        proxy_TRBC.setSafeAddresses(hostingSafe,'0x0000000000000000000000000000000000000000',{"from": multisig})

    with pytest.raises(exceptions.VirtualMachineError):
        proxy_TRBC.setSafeAddresses(hostingSafe,btcMinersSafe,{"from": person_1})

    proxy_TRBC.setSafeAddresses(hostingSafe,btcMinersSafe,{"from": multisig})

 
    assert  proxy_TRBC.hostingSafe.call() == hostingSafe
    assert  proxy_TRBC.btcMinersSafe.call() ==  btcMinersSafe

    assert  TRBC.hostingSafe.call() != hostingSafe
    assert  TRBC.btcMinersSafe.call() !=  btcMinersSafe