...
contract NodeBulls is ERC721Enumerable, IERC2981, VRFConsumerBase, Ownable {
    ...
    */
    function expand(uint256 randomValue, uint256 n) public returns (uint256[] memory expandedValues) {
        require(n <= _tokenSupply.current(), "Looking for too many results, this will infinitely loop");
        expandedValues = new uint256[](n);
        uint found_num;
        uint i = 0;
        while (found_num < n) {
            uint256 num = (uint256(keccak256(abi.encode(randomValue, i))) % _tokenSupply.current());
            if (num != 0 && num <= _tokenSupply.current() && exists(expandedValues,num) == false) {
    ...
    */
    ...
    function centennialAirDrop() public onlyOwner {
        ...
        require(checkCentennialAirdropTokenBalance(rewardTokenContract)>= 100 * 10 ** rewardDecimals, "Not Enough Funds to Warrant calling the function to award the Bulls");
        ...
        uint256 _balance = checkCentennialAirdropTokenBalance(rewardTokenContract);
        tokenContract.transfer(dev1, _balance * dev1_percent / 100);
        tokenContract.transfer(dev2, _balance * dev2_percent / 100);

        _balance = checkCentennialAirdropTokenBalance(rewardTokenContract);
    ...
    function airDropSupply() public view returns (uint256) {
        return _airDropSupply.current();
    }

    function checkCentennialAirdropTokenBalance(address token) public view returns(uint) {
    ...
    function addressPurchasedCount(address addr) external view returns (uint256) {
        return addressPurchases[addr];
    }
   ...
   function addToPresaleList(address[] calldata entries) external onlyOwner {
    ...
    function setCentennialAirDropTokenAddress(address _address) public onlyOwner {
